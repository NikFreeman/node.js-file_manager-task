...
export function initApp(){
  ...
  if (argsApp.length == 0) throw new Error(ERROR.NO_ARG)
  else if (argsApp.length >= 2) throw new Error(ERROR.MANY_ARGS)
       else store.username = argsApp[0].slice(ARG_USERNAME.length);
}
catch (e) {
